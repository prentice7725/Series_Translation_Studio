...
9. Content-based Chapter Matcher가 내용 fingerprint로 장/섹션 매칭
10. Anchor-based Paragraph Aligner가 1:1, 1:N, N:1 문단 정렬
11. 낮은 confidence 구간만 Alignment Review에서 수동 보정
12. 승인된 align pair만 TM으로 저장
13. 용어 후보 추출
14. glossary 확정
15. stylebook 초안 생성
```
...
- 매칭 기준을 파일 구조가 아니라 본문 내용, 고유명사, 숫자, 의미 anchor, 순서 정보로 둔다.
...
- content fingerprint 기반 장/섹션 매칭
- anchor 기반 문단 정렬
- 1:1, 1:N, N:1, N:M 문단 정렬 허용
- 문장 정렬 옵션
- 정렬 confidence 계산
- 낮은 confidence 구간만 수동 보정
- 낮은 confidence window만 LLM Alignment Judge로 재검증
- TM 승인/거부 UI
- 용어 후보 추출
- glossary 편집
- 캐릭터 프로필 생성
...
- 파일 번호가 다른 원서/번역본에서도 챕터 단위 95% 이상 자동 매칭 가능
...
- 영어 원문과 한국어 번역본을 content-aware 방식으로 정렬
- EPUB 내부 파일 구조를 그대로 신뢰하지 않고 Text Section 중간 구조로 정규화
- 실제 본문 시작점 탐지
- 내용 fingerprint 기반 장/섹션 매칭
- anchor 기반 문단/문장 정렬
- confidence score 계산
- 낮은 confidence 구간만 사용자가 수동 보정할 수 있는 후보 제공
...
따라서 매칭 기준은 파일 구조가 아니라 본문 내용, 고유명사, 의미, 순서 anchor다.
```
...
```text
금지:
1. 각 영어 문단마다 장 전체 한국어 문단을 대상으로 자유 검색하지 않는다.
2. HTML 파일명, spine index, 파일 번호를 강하게 믿지 않는다.
3. scene break, 공백 문단, 별표 문단을 일반 문단으로 세지 않는다.
4. 1:1 정렬만 허용하지 않는다.
5. confidence 낮은 pair를 TM에 자동 등록하지 않는다.

허용:
1. 순서 보존.
2. 1:N, N:1, N:M 허용.
3. 0:1, 1:0 skip 허용.
4. 고신뢰 anchor 사이에서만 local alignment.
5. 고유명사/숫자/대사/장면 키워드를 강하게 반영.
6. 낮은 confidence window만 LLM Judge로 보낸다.
```
...
```text
anchor 조건:
- 고유명사 2개 이상 겹침 또는 고유명사 + 숫자 겹침
- 대사 구조 또는 숫자 반복이 비슷함
- top1 점수와 top2 점수 차이가 충분함
- source 순서와 target 순서가 크게 어긋나지 않음

예:
EN: “There are fifty-six lives depending on you up there... Fifty-six is more than two.”
KO: “네 손에 56명의 목숨이 달려 있다... 56은 2보다 많다.”
```

Anchor crossing 제거:

```ts
function selectMonotonicAnchors(candidates: AnchorCandidate[]): AnchorCandidate[] {
  const strong = candidates
    .filter(c => c.score >= 0.86 && c.uniqueness >= 0.12)
    .sort((a, b) => a.srcIndex - b.srcIndex);

  return longestIncreasingByTargetIndex(strong);
}
```
...
```text
전체 장을 한 번에 DP로 밀지 않는다.
고신뢰 anchor를 먼저 고정하고 crossing anchor를 제거한 뒤,
anchor 0~1, anchor 1~2, anchor 2~3 사이의 좁은 window만 DP로 정렬한다.
이렇게 하면 한 구간에서 drift가 나도 다음 anchor에서 회복된다.
```
...
```text
1. 영어 EPUB + 한국어 EPUB/TXT 입력
2. EPUB 내부 HTML 전체 추출
3. 파일명/번호/spine index는 metadata로만 저장
4. 모든 HTML을 Text Section으로 변환
5. front matter / back matter / 역자 후기 / 감사의 글 / 광고 / 목차 후보 분리
6. Body Start Detector가 실제 본문 시작 후보 탐지
7. 사용자가 필요하면 영어 본문 시작 문단과 한국어 본문 시작 문단을 anchor로 지정
8. Content-based Chapter Matcher가 section fingerprint로 장/섹션 매칭
9. 고유명사/숫자/대사 구조 기반 고신뢰 anchor 후보 추출
10. LIS로 crossing anchor 제거
11. Anchor-based Paragraph Aligner가 anchor 사이 window만 DP 정렬
12. 1:1, 1:N, N:1, N:M, 1:0, 0:1 alignment 허용
13. confidence가 낮은 window만 Alignment Review에 표시
14. 낮은 confidence window만 LLM Alignment Judge로 재검증
15. 사용자가 승인/분할/병합/거부
16. 승인된 pair만 TM 등록
```
...
```json
{
  ...
  "target_text": "...",
  "confidence": 0.91,
  "method": "anchor+fingerprint+order",
  "status": "approved"
}
```
...
```json
{
  ...
  "confidence": 0.81,
  "needs_review": true,
  "method": "anchor+fingerprint+llm_judge",
  "status": "needs_review"
}
```
...
- 영어 본문 시작 후보
- 한국어 본문 시작 후보
- section/chapter 매칭 후보
- 낮은 confidence window

사용자가 하는 것:
- 필요할 때만 “이 둘을 1장 시작으로 연결” anchor 지정
- 낮은 confidence 구간만 확인
- AI Judge로 재검증
- 직접 anchor 추가
- 이 구간 TM 제외
```
...
```text
전체 EPUB을 LLM에게 정렬시키지 않는다.
알고리즘이 만든 낮은 confidence window만 LLM에게 보낸다.
...
- content fingerprint 기반 장/섹션 매칭
- anchor 기반 문단 매칭
- 1:1, 1:N, N:1, N:M alignment
- confidence score
- 낮은 confidence window LLM Judge 재검증
- alignment review UI
...
```text
기존 번역권 기반 Content-Aware TM 구축 가능
```
...
- [ ] content fingerprint builder
- [ ] content-based chapter/section matching
- [ ] anchor-based paragraph alignment
- [ ] 1:1 / 1:N / N:1 / N:M alignment
- [ ] confidence scoring
- [ ] low-confidence window detection
- [ ] LLM Alignment Judge for uncertain windows
- [ ] alignment review UI
